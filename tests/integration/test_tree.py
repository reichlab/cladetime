...
def test__get_tree_url(patch_s3_for_tests):
    # Use a date within data availability window (>= 2025-09-29)
    with freeze_time("2025-10-30 16:21:34"):
        ct = CladeTime()
        tree = Tree(ct.tree_as_of, ct.url_sequence)
        tree_url_parts = urlparse(tree.url)
        # Verify we get a dataset version URL with tree.json
        # (specific version will depend on what's current at 2025-10-30)
        assert "tree.json" in tree_url_parts.path
        # Verify it's a dated dataset version (format: YYYY-MM-DD--HH-MM-SSZ)
        assert "--" in tree_url_parts.path  # Date separator in version string
...
def test_tree_ncov_metadata(patch_s3_for_tests):
    ...
    with freeze_time("2025-11-15 16:21:34"):
        ...
        # sequence_as_of (2025-11-15) uses different metadata
